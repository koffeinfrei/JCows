...
/**
 * The <code>VCshort/code> class represents the Java short type.
 * 
 ...
 */
public class VCshort extends VC {
  
  private static final Logger LOGGER = Logger.getLogger(VCshort.class);
   ...
   */
  public VCshort(ParamListItem paramListItem,Composite parent) throws JCowsException {
   ...
   */
  public VCshort(ParamListItem paramListItem,Composite parent,int arrayIndex) throws JCowsException {
  ...
  private void parseValue(String value) {
    try {
      m_paramListItem.getVectorData().set(m_arrayIndex,Short.parseShort(value));
    }
  ...
  private void setDefaultParamListItemValue() throws JCowsException {
    ...
    try {
      ...
      if(m_paramListItem.getVectorData().get(0) instanceof String) {
        value=(Short)Short.parseShort((String)m_paramListItem.getVectorData().get(0));
       ...
       * response.
       */
      else if(m_paramListItem.getVectorData().get(m_arrayIndex) instanceof Short)
        value=(Short)m_paramListItem.getVectorData().get(m_arrayIndex);
      /*
       * Get default value (of correct Java type) if ParamListItem
       * holds more than one value (array mode).
       */
      else {
        value=(Short)m_paramListItem.getVectorData().get(0);
