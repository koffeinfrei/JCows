...
/**
 * The <code>VCint</code> class represents the Java integer type.
 * 
 ...
 */
public class VCint extends VC {
  
  private static final Logger LOGGER = Logger.getLogger(VCint.class);
   ...
   */
  public VCint(ParamListItem paramListItem,Composite parent) throws JCowsException {
   ...
   */
  public VCint(ParamListItem paramListItem,Composite parent,int arrayIndex) throws JCowsException {
  ...
  private void parseValue(String value) {
    try {
      m_paramListItem.getVectorData().set(m_arrayIndex,Integer.parseInt(value));
    }
  ...
  private void setDefaultParamListItemValue() throws JCowsException {
    ...
    try {
      ...
      if(m_paramListItem.getVectorData().get(0) instanceof String) {
        value=(Integer)Integer.parseInt((String)m_paramListItem.getVectorData().get(0));
       ...
       * response.
       */
      else if(m_paramListItem.getVectorData().get(m_arrayIndex) instanceof Integer)
        value=(Integer)m_paramListItem.getVectorData().get(m_arrayIndex);
      /*
       * Get default value (of correct Java type) if ParamListItem
       * holds more than one value (array mode).
       */
      else {
        value=(Integer)m_paramListItem.getVectorData().get(0);
