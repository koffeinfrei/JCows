...
/**
 * The <code>VCdouble</code> class represents the Java double type.
 * 
 ...
 */
public class VCdouble extends VC {
  
  private static final Logger LOGGER = Logger.getLogger(VCdouble.class);
   ...
   */
  public VCdouble(ParamListItem paramListItem,Composite parent) throws JCowsException {
   ...
   */
  public VCdouble(ParamListItem paramListItem,Composite parent,int arrayIndex) throws JCowsException {
    super(paramListItem,parent,arrayIndex);
    
    m_stringValue=new Text(this,SWT.NONE);
  ...
  private void parseValue(String value) {
    try {
      m_paramListItem.getVectorData().set(m_arrayIndex,Double.parseDouble(value));
    }
  ...
  private void setDefaultParamListItemValue() throws JCowsException {
    ...
    try {
      ...
      if(m_paramListItem.getVectorData().get(0) instanceof String) {
        value=(Double)Double.parseDouble((String)m_paramListItem.getVectorData().get(0));
       ...
       * response.
       */
      else if(m_paramListItem.getVectorData().get(m_arrayIndex) instanceof Double)
        value=(Double)m_paramListItem.getVectorData().get(m_arrayIndex);
      /*
       * Get default value (of correct Java type) if ParamListItem
       * holds more than one value (array mode).
       */
      else {
        value=(Double)m_paramListItem.getVectorData().get(0);
