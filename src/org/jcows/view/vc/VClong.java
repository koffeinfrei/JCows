...
/**
 * The <code>VClong</code> class represents the Java long type.
 * 
...
public class VClong extends VC {
   ...
   */
  public VClong(ParamListItem paramListItem,Composite parent) throws JCowsException {
   ...
   */
  public VClong(ParamListItem paramListItem,Composite parent,int arrayIndex) throws JCowsException {
  ...
  private void parseValue(String value) {
    try {
      m_paramListItem.getVectorData().set(m_arrayIndex,Long.parseLong(value));
    }
  ...
  private void setDefaultParamListItemValue() throws JCowsException {
    ...
    try {
      ...
      if(m_paramListItem.getVectorData().get(0) instanceof String) {
        value=(Long)Long.parseLong((String)m_paramListItem.getVectorData().get(0));
       ...
       * response.
       */
      else if(m_paramListItem.getVectorData().get(m_arrayIndex) instanceof Long)
        value=(Long)m_paramListItem.getVectorData().get(m_arrayIndex);
      /*
       * Get default value (of correct Java type) if ParamListItem
       * holds more than one value (array mode).
       */
      else {
        value=(Long)m_paramListItem.getVectorData().get(0);
